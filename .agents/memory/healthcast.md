...
## Multi-clip audio playback (Welcome/onboarding narrated tour)
- Pattern that survived code review: a `sessionRef` (monotonic counter) is the single invalidation boundary for ALL async media work. `stopAllAudio()` bumps it (+ clears handlers, pauses, resets both `<audio>` elements) and runs on unmount, mode-switch, error, and end-of-tour.
- A fresh-start play (`playAudioForIndex(index)`) CLAIMS a new session (`++sessionRef.current`); a RESUME after pause CAPTURES the current value (no bump) so the existing timeupdate/ended/error handlers stay valid. Every `.then/.catch` and every persisted handler bails if `session !== sessionRef.current`.
- **Why:** without this, an in-flight `play()` promise (and the recurring music bed) resumes audio AFTER stop/mode-switch/unmount, leaking sound. Two-attempt lesson: also DON'T call `pause()` in a stale async branch — the shared element may already be owned by a newer session; just no-op.
- Pass the target index EXPLICITLY into the play fn; never read `currentIndex` from a stale closure. Chapter clicks set index first, then play that exact index. Read `mode`/`isMuted` from refs inside the play fn to avoid stale closures. Mute toggles write `.muted` directly on the elements (no restart).
- Assets are static files under `public/onboarding/` referenced as `${import.meta.env.BASE_URL}onboarding/<file>` (BASE_URL includes trailing slash). Verify via proxy `curl localhost:80/onboarding/<file>` (200), NOT `$REPLIT_DEV_DOMAIN` from bash (connection refused). PDF guide is generated by a runnable pdfkit script in `@workspace/scripts`.
...
## "Executive Contrast" theme architecture (light content + navy chrome)
- Theme lives entirely in `src/index.css` token layers — NO `.dark` class toggle is used. `:root` = LIGHT content tokens; a `header, aside { ... }` rule re-declares background/foreground/card/muted/secondary/border/primary/cyan AND both surface gradients (`--gradient-surface`, `--gradient-surface-accent`) to navy, so every chrome descendant resolves dark automatically without per-element classes.
- **Why this works / the landmine:** the element-scoped overrides only stay confined to chrome because ONLY `TopHeader` renders `<header>` and ONLY `SidebarNav` renders `<aside>` (PageHeader uses `<div>`). If any content component ever uses a semantic `<header>`/`<aside>`, it will inherit navy tokens and break. Keep content blocks on `<div>`/`<section>`.
- **Content surface gradients must stay LIGHT in `:root`** even though the source mockup kept them navy: real content cards (KpiCard/ChartCard/InsightCard/RecommendedAction) consume `surface-gradient`/`surface-gradient-accent` utilities, so light values at root are required; the chrome scope overrides them back to navy.
- Chrome accent rules must carry an extra class to beat Tailwind utilities (specificity): `header.surface-gradient-accent` (teal underline+shadow, beats `shadow-lg`/`border-border`) and `aside .bg-gradient-primary` (active-nav glow).
- Colorful KPI treatment on ExecutiveOverview is data-order-coupled: `kpiIcons` + `kpiAccents` arrays index-align with `data/overviewData.ts topKpis`. DonutChart takes an optional `palette?: string[]` to override per-slice `entry.fill` (used for the multi-hue revenue donut + matching legend swatches).
